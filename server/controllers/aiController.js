...
async function ensureFlashcardStatusColumn() {
  ...
  if (rows.length === 0) {
    await db.execute(
      "ALTER TABLE flashcards ADD COLUMN status ENUM('active','archived') DEFAULT 'active'",
    );
  }
...
async function getMaterialText(materialId, userId) {
  ...
  if (rows[0].status === "processing")
    throw new Error("Material is still processing. Please wait.");
    ...
    for (const card of cards) {
      const [result] = await db.execute(
        ...
        [
          ...
          card.question,
          card.answer,
          card.topic || topic || material.title,
...
async function getFlashcards(req, res) {
  try {
    ...
    let query =
      "SELECT f.*, s.subject_name FROM flashcards f LEFT JOIN subjects s ON f.subject_id = s.id WHERE f.user_id = ?";
...
async function deleteFlashcard(req, res) {
  try {
    await db.execute("DELETE FROM flashcards WHERE id = ? AND user_id = ?", [
      req.params.id,
      req.user.id,
    ]);
...
async function submitQuiz(req, res) {
  try {
    ...
    if (!session_id || !answers)
      return res
        .status(400)
        .json({ message: "session_id and answers required" });
    ...
    for (const a of answers) {
      const [qRows] = await db.execute(
        "SELECT answer, explanation FROM quiz_questions WHERE id = ?",
        [a.question_id],
      );
...
async function predictExam(req, res) {
  try {
    ...
    let query =
      "SELECT extracted_text, title FROM materials WHERE user_id = ? AND status = 'ready'";
    const params = [req.user.id];
    if (subject_id) {
    ...
    if (material_ids?.length) {
      query += ` AND id IN (${material_ids.map(() => "?").join(",")})`;
    ...
    if (materials.length === 0)
      return res
        .status(400)
        .json({ message: "No processed materials found for prediction" });
...
Return ONLY valid JSON:
{
  ...
  "study_advice": "..."
    ...
    const raw = await callAI(
      systemPrompt,
      `Analyze these course materials and predict the most likely exam topics:\n\n${combinedText}`,
      true,
    );
    const result = JSON.parse(raw);
    ...
    if (subject_id) {
      await db.execute(
        ...
        [
          ...
          JSON.stringify(result.predictions.map((p) => p.confidence)),
        ],
      );
    }
...
Return ONLY valid JSON:
{
  ...
  "study_plan_suggestion": "..."
    ...
    const raw = await callAI(
      systemPrompt,
      `Student data: ${JSON.stringify(contextData)}`,
      true,
    );
...
async function getProgress(req, res) {
  try {
    ...
    res.json({
      ...
      weekly,
      flashcard_stats: flashcardStats[0],
    });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
...
async function generateCalendar(req, res) {
  try {
    ...
    const [user] = await db.execute(
      "SELECT daily_study_hours, study_preference FROM users WHERE id = ?",
      [userId],
    );
...
Tasks to schedule: ${JSON.stringify(tasks.map((t) => ({ title: t.title, subject: t.subject_name, deadline: t.deadline, hours: t.estimated_hours, difficulty: t.difficulty })))}`;
    ...
    res.json({
      message: `${saved.length} calendar events generated`,
      events: saved,
    });
  } catch (error) {
...
Return ONLY valid JSON:
{
  ...
  "ai_tips": ["tip1", "tip2", "tip3"]
    ...
    const raw = await callAI(
      systemPrompt,
      `Generate ${days}-day timetable from ${targetDate}. Tasks: ${JSON.stringify(
        tasks.map((t) => ({
          title: t.title,
    ...
    if (result.timetable) {
      try {
        await db.execute(
          "DELETE FROM timetable_slots WHERE user_id = ? AND date >= ?",
          [userId, targetDate],
        );
        ...
        console.warn("Timetable DB save skipped:", dbErr.message);
      }
    }

    res.json({
      ...result,
      message: `${days}-day timetable at ${difficultyLevel} level`,
    });
  } catch (error) {
...
async function getTimetable(req, res) {
  try {
    ...
    try {
      ...
      res.json({
        timetable,
        weekly_summary: "Saved AI timetable loaded.",
        ai_tips: [],
      });
    } catch (e) {
...
async function assessDifficulty(req, res) {
  try {
    ...
    const composite = Math.round(
      avgScore * 0.5 + flashAccuracy * 0.3 + completionRate * 0.2,
    );
    ...
    if (composite >= 80) {
      ...
      next_level_at = null;
      suggestion =
        "Outstanding! Push yourself with harder materials and aim for 90%+ on quizzes.";
      ...
      next_level_at = 80;
      suggestion =
        "Solid progress! Review weak subjects daily and complete all flashcard sets.";
    } else {
      level = "beginner";
      label = "Beginner";
      color = "#4a8fe8";
      next_level_at = 55;
      suggestion =
        "Great start! Consistency is everything — even 30 minutes daily makes a huge difference.";
    }

    try {
      await db.execute("UPDATE users SET difficulty_level = ? WHERE id = ?", [
        level,
        userId,
      ]);
    } catch (e) {}
    ...
    res.json({
      ...
      color,
      scores: {
        quiz_average: Math.round(avgScore),
        flashcard_accuracy: Math.round(flashAccuracy),
        task_completion: completionRate,
        composite,
      },
      next_level_at,
      suggestion,
      sessions_tracked: perf[0]?.sessions || 0,
    });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
...
async function prioritizeTasks(req, res) {
  try {
    ...
    if (tasks.length === 0)
      return res.json({ tasks: [], message: "No pending tasks" });
    ...
    const scored = tasks
      .map((t) => {
        const daysLeft = t.deadline
          ...
          : 30;
        const diffScore =
          { easy: 20, medium: 50, hard: 80 }[t.difficulty] || 50;
        const urgency =
          daysLeft === 0
            ? 100
            : daysLeft <= 1
    ...
    for (const t of scored) {
      await db.execute("UPDATE tasks SET priority_score = ? WHERE id = ?", [
        t.priority_score,
        t.id,
      ]);
    }
    ...
    res.json({
      tasks: scored,
      ai_note: `Prioritized based on deadlines, difficulty, and your avg score of ${Math.round(avgPerf)}%`,
    });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
  ...
  try {
    ...
    const subjectAnalysis = subjects.map((subject) => {
      ...
      if (subjectFlashcards && subjectFlashcards.unreviewed_cards > 0) {
        urgencyScore += 10;
        urgencyReason.push(
          `${subjectFlashcards.unreviewed_cards} unreviewed flashcards`,
        );
      }
    ...
    const rankedSubjects = subjectAnalysis.sort(
...
async function generateStudyPlan(req, res) {
  try {
    ...
    if (!subject_id && !material_id) {
      return res
        .status(400)
        .json({ message: "Provide subject_id or material_id" });
    }
    ...
    if (subjectPerf.length === 0) {
      return res.json({
        suggestions: [
          {
            message:
              "Start by creating subjects to get personalized topic recommendations.",
